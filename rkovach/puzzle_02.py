...
def part_one(input_):
    h_pos = 0
    v_pos = 0
    for movement in input_.splitlines():
        action, offset = movement.split()
        offset = int(offset)
        if action == 'forward':
            h_pos += offset
        elif action == 'down':
...
assert part_one(test_input) == 150, 'Expected answer is 150.'
assert part_two(test_input) == 900, 'Expected answer is 900.'
