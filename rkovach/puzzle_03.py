...
def count_bits(input_):
    numbers = input_.splitlines()
    bits = [(int(bit)) for bit in numbers[0]]
    bits = [[x] for x in bits]
    
    for number in numbers[1:]:
        bits1 = [int(bit) for bit in number]
    ...
    for i, b in enumerate(bits):
        if bits[i].count(1) >= bits[i].count(0):
            gamma += '1'
            epsilon += '0'
        else:
            gamma += '0'
...
def filter_numbers(numbers, index):
    """Filter out numbers based on the most common value
    in each position."""

    filtered = []
    bits = [int(num[index]) for num in numbers]
    filter_ = 1 if bits.count(1) >= bits.count(0) else 0
    for n in numbers:
        if int(n[index]) == filter_:
            filtered.append(n)

    if len(filtered) > 1:
        index += 1
        return filter_numbers(filtered, index)
    return filtered

def filter_numbers2(numbers, index):
    """Filter out numbers based on the least used value
    in each position."""

    filtered = []
    bits = [int(num[index]) for num in numbers]
    filter_ = 0 if bits.count(0) <= bits.count(1) else 1
    for n in numbers:
        if int(n[index]) == filter_:
            filtered.append(n)
    ...
    if len(filtered) > 1:
        index += 1
        return filter_numbers2(filtered, index)
    return filtered


def part_two(input_):
    numbers = input_.splitlines()
    oxy = filter_numbers(numbers, 0)[0]
    co2 = filter_numbers2(numbers, 0)[0]

    return int(oxy, 2) * int(co2, 2)


assert count_bits(test_input) == 198
assert part_two(test_input) == 230

print(f'Part One: {count_bits(puzzle_input)}')
