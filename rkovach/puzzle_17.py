...
puzzle_input = "target area: x=230..283, y=-107..-57"


def get_target_area(input_):
    x_min = x_max = y_min = y_max = 0
    tokens = input_.split()
    for t in tokens:
        if 'x' in t:
            axis, range_ = t.split('=')
            x_min, x_max = range_.split('..')
        elif 'y' in t:
            axis, range_ = t.split('=')
            y_min, y_max = range_.split('..')
    return( int(x_min), int(x_max.replace(',', '')), int(y_min), int(y_max))


def fire_shot(xv, yv, target):
    x1, x2, y1, y2 = target
    peak_position = 0
    position = [0, 0]
    hit = False
    while position[0] <= x2 and position[1] >= y1:

        peak_position = max(peak_position, position[1])
...
def run(input_):
    target = get_target_area(input_)
    c = 0
    hits = []
    for x in range(0, 400, 1):
        for y in range(-200, 200, 1):
            result = fire_shot(x, y, target)
