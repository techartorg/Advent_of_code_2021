...
input). Crab submarines have limited fuel, so you need to find a way to make all
 of their horizontal positions match while requiring them to spend as little 
 fuel as possible.
...
def part_one(positions):
    ...
    for i in range(h_min, h_max + 1):
        ...
        for p in positions:
            fuel += abs(p - i)
        spent_fuel.append(fuel)
    return min(spent_fuel)


assert part_one(parse_input(test_input)) == 37
...
def part_two(positions):
    ...
    # start with a fuel cost for the average, then we can early out
    # when testing values at the extremes
    average = int(sum(positions) / len(positions))
    fuel_cost = 0

    # initialize the fuel_cost variable with the fuel needed to reach the
    # center point. Each loop will update this value is the fuel cost is
    # lower.
    for p in positions:
        steps = abs(p - average)
